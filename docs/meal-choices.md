...
No added package, database migration or subscription. Published to Google Play internal testing in [version 1.0.7](testing-release-1.0.7.md).

Validation: TypeScript, 111 unit tests, and `scripts/verify-meal-choices-ui.cjs` against a local Expo preview. The browser check intercepts account requests and mutations, uses synthetic account data, and only reads the public recipe catalog. It covers all four home buttons, hold/release/refresh, reopening, favorites, returning from details, empty/small catalogs, loading failure and retry, 320/390/844-pixel layouts and dark mode. Native Android interactions remain part of internal tester review on phones.
