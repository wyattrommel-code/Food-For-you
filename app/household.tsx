...
import {HouseholdPlanningSettings} from '@/components/HouseholdPlanningSettings';
...
export default function HouseholdScreen(){
 ...
 const confirmLeave=()=>{if(pending.length){Alert.alert('Changes are waiting to sync','Reconnect and sync your grocery changes before leaving the household.');return;}Alert.alert('Leave household?',household?.ownerId===userId?'Your preference sharing stops. The next member becomes the owner. If you are the last member, the shared planner and groceries will be deleted. Your personal planner stays private.':'You will lose access to shared plans and groceries, and stop sharing preferences. Your personal planner stays private.',[{text:'Cancel',style:'cancel'},{text:'Leave',style:'destructive',onPress:()=>void run('leave')}]);};
 ...
 <Text style={{fontSize:16,lineHeight:24,color:Colors.textSecondary}}>Separate logins, meals together. Share groceries, opt into a household planner, and choose whether to share your food preferences.</Text>
