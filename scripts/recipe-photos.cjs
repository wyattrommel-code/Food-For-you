...
function photoAfter(entry) {
  return { ...entry.before, image_url: entry.image.url, description: entry.before.description.trimEnd() + '\n\n' + entry.image.credit };
}
function validateManifest(manifest) {
  ...
  for (const e of manifest.entries) {
    ...
    assert.equal(e.review.status, 'visually-reviewed'); assert.ok(e.review.notes.trim());
    ...
    assert.ok(['generated', 'licensed-photo', 'owned-photo'].includes(e.image.kind));
    if (e.image.kind === 'generated') {
      ...
      assert.ok(['CC0', 'CC BY 2.0', 'CC BY 3.0', 'CC BY 4.0', 'CC BY-SA 3.0', 'CC BY-SA 4.0', 'Pexels License', 'Unsplash License'].includes(e.image.license), 'Review the reuse license');
...
if (require.main === module) {
  ...
  const manifest = JSON.parse(fs.readFileSync(path.join(root, 'data/recipes/photo-manifest.json'), 'utf8'));
  validateFiles(manifest, root);
  if (process.argv.includes('--sql')) fs.writeFileSync(path.join(root, 'data/recipes/photo-manifest.sql'), importSql(manifest));
  if (process.argv.includes('--verify-remote')) verifyRemote(manifest).then(()=>console.log(`Verified ${manifest.entries.length} published JPEGs and hashes.`)).catch(e=>{ console.error(e.message);process.exitCode=1; });
  else console.log(`Validated ${manifest.entries.length} reviewed photos and local files. No network calls or database writes.`);
}
