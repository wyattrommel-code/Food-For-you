import {dateKey,parseDay,shiftDay,weekDays, type PlanEntry} from './planner';
...
export function inShoppingScope(date:string,scope:ShoppingScope,day:string,today:string){if(scope==='all')return date>=today;if(scope==='day')return date===day;const week=weekDays(day);return date>=week[0]&&date<=week[6];}
...
export function plannedShopping(plans:PlannedRecipe[],checks:Purchase[],scope:ShoppingScope,day:string,today:string):PlannedItem[]{
 const purchased=new Map(checks.map(c=>[purchaseKey(c),c.checked]));const items=new Map<string,PlannedItem>();
 for(const plan of plans.filter(p=>inShoppingScope(p.plan_date,scope,day,today))){
